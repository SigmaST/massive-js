...
Table.prototype.update = function (criteria, fields, options = {}) {
  if (_.isNil(fields)) {
    ...
    criteria = _.reduce(this.pk, (memo, k) => {
      memo[k] = fields[k];
      return memo;
    }, {});
 ...
 */
Table.prototype._save = function (record, options = {}) {
...
Table.prototype.save = function (obj, fields, options = {}) {
  if (!_.isObject(obj)) {
    return this._save(_.extend(fields, {
      [this.pk[0]]: obj
    }), options);
  }

  return this._save(obj, fields || options);
};

Table.prototype.hasPk = function (record) {
  const keys = _.keys(record);
  return _.reduce(this.pk, (m, k) => {
    return m && (keys.indexOf(k) > -1 || keys.indexOf(`"${k}"`) > -1);
  }, true);
};
