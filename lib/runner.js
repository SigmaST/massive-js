    ...
    if(err){
      ...
      db.query(args.sql, args.params, function (err, result) {
        ...
        if (err) {
          ...
          } else if (args.options.paginate && args.options.paginate.count) {
            var name = args.options.paginate.name || "paginate";
            var prop = args.options.paginate.count;
            result.rows[name] = {
              limit: args.options.paginate.limit,
              offset: args.options.paginate.offset
            };
            result.rows[name][prop] = (result.rows[0] || {})[prop] || 0;
          }
