...
Queryable.prototype.find = function(){
  ...
  var sql = "select " + cols;
  if (options.paginate) {
    if (_.isObject(options.paginate)) {
      options.paginate = {
        name: options.paginate.name,
        count: options.paginate.count || "count"
      };
    } else {
      options.paginate = {
        count: (_.isString(options.paginate) ? options.paginate : "count")
      };
    }
    options.paginate.limit = options.limit;
    options.paginate.offset = options.offset || 0;
    sql += ", (SELECT COUNT(*) FROM " + this.delimitedFullName + where.where +") AS " + options.paginate.count;
  }
  sql += " from " + this.delimitedFullName + where.where;
  ...
  if (args.options.stream) {
    ...
    this.db.query(sql, where.params, {single: returnSingle, paginate: options.paginate}, args.next);
  }
};
