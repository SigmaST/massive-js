...
Queryable.prototype.findOne = function() {
  ...
  this._find(args.conditions, args.query, function(err,results) {
    if (err) {
      args.next(err,null);
    } else {
      var result;
...
Queryable.prototype.find = function() {
  if(arguments && arguments[1] && !_.isFunction(arguments[1])){
    if(!_.isObject(arguments[1])){
      arguments[1] = {offset: arguments[1]};
    }
    arguments[1] = _.extend({
      order: "1 DESC",
      limit: 20,
      offset: 0,
      paginate: true
    }, arguments[1]);
    arguments[1].offset = ((arguments[1].offset || 0) * arguments[1].limit) || 0;
  }
...
Queryable.prototype._find = function() {
  var args = ArgTypes.findArgs(arguments, this);
...
Queryable.prototype._findSync = DA(Queryable.prototype._find);
