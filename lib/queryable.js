...
Queryable.prototype.findOne = function() {
  ...
  this.find(args.conditions, args.options, function(err,results) {
    if (err) {
      args.next(err,null);
    } else {
      var result;
...
Queryable.prototype.find = function() {
  ...
  var sql = "select " + args.options.selectList(where) + " from " + this.delimitedFullName + where.where + args.options.queryOptions();
