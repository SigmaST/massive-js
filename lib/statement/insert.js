...
Insert.prototype.format = function () {
  ...
  let sqlUser = '';
  if (this.user || this.source.db.user) {
    sqlUser += `WITH application_user AS (SELECT ${this.user || this.source.db.user}) `;
  }

  let sql = `${sqlUser}INSERT INTO ${this.source.delimitedFullName} (${quotedColumns.join(', ')}) VALUES `;
