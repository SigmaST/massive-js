...
const Select = function (source, criteria = {}, options = {}) {
  options = _.defaults(options, {
    ...
    single: false,
    paginate: !(options.single || (options.limit || '').toString().toUpperCase() === 'ALL'),
    count: 'count',
    limit: 20,
    order: '1 DESC',
    offset: 0
  ...
  if (this.paginate) {
    ...
    // this.columns = `${this.columns}, (SELECT COUNT(*) FROM ${source.delimitedFullName} ${this.where}) AS ${options.count}`;
    this.columns.push(`(SELECT COUNT(*) FROM ${source.delimitedFullName} WHERE ${this.where.conditions}) AS ${options.count}`);
  }
};
