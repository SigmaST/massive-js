...
const Readable = function (spec) {
  ...
  if (spec.paginate) {
    this.paginate.active = true;
    if (_.isString(spec.paginate)) {
      this.paginate.name = spec.paginate;
    } else if (_.isPlainObject(spec.paginate)) {
      this.paginate = _.defaults(spec.paginate, this.paginate);
    }
  }
};
...
Readable.prototype.find = function (criteria = {}, options = {}) {
  if (_.isBoolean(options.paginate)) {
    options.paginate = _.defaults({active: options.paginate}, this.paginate);
  } else if (_.isString(options.paginate)) {
    options.paginate = _.defaults({name: options.paginate}, this.paginate);
  } else if (_.isPlainObject(options.paginate)) {
    options.paginate = _.defaults(options.paginate, this.paginate);
  } else {
    options.paginate = _.defaults({}, this.paginate);
  }

  const paginate = !options.single && options.limit !== 'ALL' && options.limit !== 'all' && !!options.paginate.active;

  if (paginate) {
    if (!options.limit) {
      options.limit = options.paginate.limit;
    }

    if (!options.offset && _.isNumber(options.page)) {
      options.offset = options.page * options.limit;
    }
  }

  const select = new Select(this, criteria, options);
  ...
  if (paginate) {
    promises.push(this.db.query(new Select(this, criteria, {exprs: {count: 'COUNT(1)'}, order: null, single: true})));
  }

  return Promise.all(promises).then(([res1, res2]) => {
    if (paginate) {
      res1[options.paginate.name] = {
        limit: select.limit,
