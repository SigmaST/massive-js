...
const Query = function (conditions = {}, options = {}, object = {}) {
  ...
  if (options.paginate) {
    this.paginate = _.extend({
      ...
      limit: this.limit,
      offset: this.offset
    }, (_.isString(options.paginate) ? { count: options.paginate } : options.paginate));
    this.paginate.sql = util.format("(SELECT COUNT(*) FROM %s %s) AS %s", object.delimitedFullName, this.where.conditions, this.paginate.count);
    this.columns = `${this.columns}, ${this.paginate.sql}`;
  }
};
